...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        with pyodbc.connect('DRIVER='+driver+';SERVER=tcp:'+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD=' + password) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT genre, AVG(runtimeMinutes), STDEV(runtimeMinutes) FROM [dbo].[tGenres] LEFT JOIN [dbo].[tTitles] ON [dbo].[tGenres].tconst = [dbo].[tTitles].tconst WHERE runtimeMinutes >= 0 GROUP BY genre")
